...
LIMITS = Namespace(
    ...
    settings=[
        SettingSpec(
            ...
            "Designer agent max model turns",
            "int",
            12,
            min=1,
            max=50,
            description=(
                "Soft ceiling on LLM turns; agent should finish early when done "
                "(does not pad to this count)."
            ),
        ),
        SettingSpec(
            "DESIGNER_AGENT_MAX_GENERATIONS",
            "Designer max candidate generations",
            "int",
            3,
            min=1,
            max=12,
        ...
        SettingSpec(
            "DESIGNER_AGENT_MAX_SEARCHES",
            "Designer max catalog searches",
            "int",
            12,
            min=1,
            max=40,
            description="Combined ai_search + keyword_search calls per job.",
        ),
        SettingSpec(
            "DESIGNER_AGENT_WALL_TIME_SECONDS",
            "Designer wall-clock budget (seconds)",
            "int",
            900,
            min=0,
            max=7200,
            description="Hard job timeout; 0 = unlimited. Default 15 minutes.",
        ),
        SettingSpec(
            "DESIGNER_AGENT_SEARCH_LIMIT",
            "Designer search hits per query",
            "int",
            12,
            min=1,
            max=15,
            description="Default top-K catalog hits returned per ai/keyword search (model chooses).",
        ),
        SettingSpec(
            "DESIGNER_AGENT_MAX_REFERENCES",
            "Designer max refs per generation",
            "int",
            7,
            min=1,
            max=12,
            description="Safety cap on catalog references per generate_image call.",
        ),
        SettingSpec(
            "DESIGNER_AGENT_PASS_SCORE",
            "Designer evaluate pass score",
            "float",
            7.5,
            min=0,
            max=10,
        ...
        SettingSpec(
            "CLEANUP_ENABLED",
            "In-process weekly cleanup",
            "bool",
            "true",
            "When true, lifespan starts a background task that purges expired guest gens, "
            "old designer job rows, expired OTPs/JTIs. Multi-worker safe via Postgres advisory lock.",
        ),
        SettingSpec(
            ...
            min=1,
            max=90,
            description="Minimum days between successful scheduled cleanup runs.",
        ),
        SettingSpec(
            ...
            min=1,
            max=365,
            description="Guest generations must be older than this before delete (and expired/unclaimed).",
        ),
        SettingSpec(
            "DESIGNER_JOB_RETENTION_DAYS",
            "Designer job retention (days)",
            "int",
            7,
            min=1,
            max=365,
            description="Delete terminal designer_jobs rows older than this (updated_at).",
        ),
        SettingSpec(
            "DESIGNER_JOB_STALE_RUNNING_HOURS",
            "Designer job stale hours",
            "int",
            2,
            min=1,
            max=168,
            description="Mark non-terminal designer jobs failed when updated_at older than this.",
        ),
        SettingSpec(
            "INTERNAL_DESIGNER_RATE_LIMIT",
            "Internal designer API rate limit",
            "int",
            40,
            min=1,
            max=500,
            description="Max POST /internal/designer/jobs per user per window (bot-hostile, human-ok).",
        ),
        SettingSpec(
            "INTERNAL_DESIGNER_RATE_WINDOW_SECONDS",
            "Internal designer rate window (s)",
            "int",
            3600,
            min=60,
            max=86400,
            description="Fixed window length for INTERNAL_DESIGNER_RATE_LIMIT.",
        ),
        SettingSpec(
            "INTERNAL_DESIGNER_MAX_CONCURRENT",
            "Internal designer max concurrent jobs",
            "int",
            10,
            min=1,
            max=50,
            description="Max in-flight designer jobs per user via the internal tools API.",
        ),
        SettingSpec(
            "INTERNAL_DESIGNER_PROMPT_MAX_CHARS",
            "Internal designer prompt max chars",
            "int",
            8000,
            min=500,
            max=50000,
        ...
        SettingSpec(
            "INTERNAL_DESIGNER_WAIT_TIMEOUT_SECONDS",
            "Internal designer wait-mode timeout (s)",
            "int",
            900,
            min=30,
            max=3600,
            description=(
                "Max seconds POST /internal/designer/jobs?wait=true (or form wait=true) "
                "blocks before returning job_id with non-terminal status for polling."
            ),
        ),
    ],
)
...
DEBUG = Namespace(
    ...
    settings=[
        ...
        SettingSpec(
            ...
            "Langfuse / OTEL service name",
            "string",
            "ducon-library-backend",
            "Shown in Langfuse instead of unknown_service. Also sets OTEL_SERVICE_NAME when unset.",
        ),
    ],
)
...
SECRETS = Namespace(
    ...
    settings=[
        ...
        SettingSpec(
            "INTERNAL_DESIGNER_API_KEY",
            "Internal designer tools API key",
            "string",
            "",
            is_secret=True,
            editable=False,
